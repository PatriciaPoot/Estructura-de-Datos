...
class EditorGrafosDijkstraStyle:
    def __init__(self, root):
        self.root = root
        self.root.title("Editor de Grafos - Dijkstra Interactivo")
    ...
    def _crear_menu(self):
        ...
        tk.Button(menu, text="AGREGAR NODO", command=self.agregar_nodo, 
                 bg="#bde0fe").pack(pady=5, fill=tk.X, padx=20)
        ...
        tk.Button(menu, text="CONECTAR", command=self.agregar_arista, 
                 bg="#baffc9").pack(pady=5, fill=tk.X, padx=20)
        self._crear_separador(menu)
        
        self._crear_seccion(menu, "⚡ RUTA MÁS CORTA")
        tk.Label(menu, text="Desde:", bg="#252526", fg="white").pack()
        self.ent_desde = tk.Entry(menu, justify='center', font=("Segoe UI", 10))
        self.ent_desde.pack(pady=2, padx=20, fill=tk.X)
        
        tk.Label(menu, text="Hasta:", bg="#252526", fg="white").pack()
        self.ent_hasta = tk.Entry(menu, justify='center', font=("Segoe UI", 10))
        self.ent_hasta.pack(pady=2, padx=20, fill=tk.X)
        
        tk.Button(menu, text="BUSCAR RUTA", command=self.calcular_dijkstra, 
                 bg="#ffcce6").pack(pady=5, fill=tk.X, padx=20)
        tk.Button(menu, text="RUTAS DESDE NODO", command=self.calcular_rutas_todas, 
                 bg="#ffe6cc").pack(pady=5, fill=tk.X, padx=20)
        self._crear_separador(menu)
        
        self._crear_seccion(menu, "✎ EDICIÓN")
        tk.Button(menu, text="EDITAR NOMBRE", command=self.editar_nodo, 
                 bg="#ffffba").pack(pady=5, fill=tk.X, padx=20)
        tk.Button(menu, text="ELIMINAR NODO", command=self.eliminar_nodo, 
                 bg="#ff9aa2").pack(pady=5, fill=tk.X, padx=20)
        tk.Button(menu, text="ELIMINAR ARISTA", command=self.eliminar_arista, 
                 bg="#ffb3ba").pack(pady=5, fill=tk.X, padx=20)
        self._crear_separador(menu)
        
        self._crear_seccion(menu, "📊 INFORMACIÓN")
        tk.Button(menu, text="VACIAR PROGRAMA", command=self.vaciar_grafo, 
                 bg="#d8bfff").pack(pady=5, fill=tk.X, padx=20)
        tk.Button(menu, text="SALIR", command=self.root.quit, 
                 bg="#ff6b6b", fg="white").pack(pady=20, fill=tk.X, padx=20)
    ...
    def agregar_nodo(self):
        ...
        if not nombre.replace("_", "").replace("-", "").isalnum():
            messagebox.showwarning("Error", "Caracteres no válidos")
            return
    ...
    def eliminar_arista(self):
        u = simpledialog.askstring("Eliminar arista", "Nodo origen:", parent=self.root)
        if not u or u not in self.G: 
            return
        v = simpledialog.askstring("Eliminar arista", "Nodo destino:", parent=self.root)
    ...
    def calcular_dijkstra(self):
        ...
        if not u or not v: 
            messagebox.showwarning("Error", "Complete campos")
            return
        if u not in self.G or v not in self.G: 
            messagebox.showwarning("Error", "Nodo no existe")
            return
        
        try:
            camino = nx.dijkstra_path(self.G, u, v)
            dist = nx.dijkstra_path_length(self.G, u, v)
            self.camino_resaltado = list(zip(camino, camino[1:]))
            self.actualizar_dibujo()
            messagebox.showinfo("Dijkstra", f"Ruta: {' → '.join(camino)}\nDistancia: {dist}")
            for e in [self.ent_desde, self.ent_hasta]: 
                e.delete(0, tk.END)
        except:
            messagebox.showinfo("Info", "No existe ruta")
    
    def calcular_rutas_todas(self):
        origen = simpledialog.askstring("Rutas desde nodo", "Nodo origen:", parent=self.root)
        if origen and origen in self.G:
            try:
                distancias = nx.single_source_dijkstra_path_length(self.G, origen)
                if len(distancias) > 1:
                    msg = f"Rutas desde {origen}:\n\n" + "\n".join(f"→ {d}: {p}" for d, p in sorted(distancias.items()) if d != origen)
                    messagebox.showinfo("Distancias", msg)
            except:
                pass
    ...
    def actualizar_dibujo(self):
        ...
        for u, v, d in self.G.edges(data=True):
            ...
            xs, ys = x1 + (dx/dist)*radio, y1 + (dy/dist)*radio
            xe, ye = x2 - (dx/dist)*radio, y2 - (dy/dist)*radio
            
            es_camino = (u, v) in self.camino_resaltado or (not self.es_dirigido and (v, u) in self.camino_resaltado)
            ...
            self.ax.text(mx - dy/dist*0.03, my + dx/dist*0.03, str(d['weight']),
                        color="#ffcc99", fontsize=11, fontweight='bold',
                        ha='center', va='center')
...
if __name__ == "__main__":
    root = tk.Tk()
    app = EditorGrafosDijkstraStyle(root)
    root.mainloop()
