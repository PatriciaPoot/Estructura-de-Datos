...
class EditorGrafosFloydStyle:
    ...
    def calcular_floyd(self):
        ...
        ventana.title("Floyd - Distancias Mínimas")
        ...
        tk.Label(cont, text="DISTANCIAS MÍNIMAS (FLOYD)", 
                font=("Segoe UI", 14, "bold"), bg="#1e1e1e", fg="#ffcc99").pack(pady=(0,10))
        columnas = 4
        num_nodos = len(nodos)
        nodos_por_columna = (num_nodos + columnas - 1) // columnas
        frame_columnas = tk.Frame(cont, bg="#1e1e1e")
        frame_columnas.pack()
        for col in range(columnas):
            col_frame = tk.Frame(frame_columnas, bg="#1e1e1e")
            col_frame.pack(side=tk.LEFT, padx=15)
            inicio = col * nodos_por_columna
            fin = min((col + 1) * nodos_por_columna, num_nodos)
            for idx in range(inicio, fin):
                u = nodos[idx]
                tk.Label(col_frame, text=f"┌─ Desde {u} ─────────────────────", 
                        font=("Consolas", 10, "bold"), bg="#1e1e1e", fg="#888888").pack(anchor="w")
                for v in nodos:
                    valor = dist[u][v]
                    if valor == float('inf'):
                        texto = f"│  {u} → {v}: ✗ NO HAY CAMINO"
                        color = "#ff8888"
                    elif u == v:
                        texto = f"│  {u} → {v}: 0 (mismo nodo)"
                        color = "#ffcc99"
                    else:
                        texto = f"│  {u} → {v}: {round(valor, 2)}"
                        color = "#88ff88"
                    tk.Label(col_frame, text=texto, font=("Consolas", 9), 
                            bg="#1e1e1e", fg=color).pack(anchor="w")
                tk.Label(col_frame, text="└───────────────────────────────", 
                        font=("Consolas", 10), bg="#1e1e1e", fg="#888888").pack(anchor="w", pady=(0,10))
        tk.Button(cont, text="ACEPTAR", command=ventana.destroy,
                  bg="#764ba2", fg="white", font=("Segoe UI", 10, "bold"),
                  padx=20, pady=5).pack(pady=20)
        cont.update_idletasks()
        canvas.configure(scrollregion=canvas.bbox("all"))
        w = min(cont.winfo_reqwidth() + 40, ventana.winfo_screenwidth() - 100)
        h = min(cont.winfo_reqheight() + 40, ventana.winfo_screenheight() - 100)
